...
class _AsyncCheckpointProcess:
    ...
    def __del__(self) -> None:
        if self._save_process.is_alive():
            logger.info(f?"Terminating the checkpoint background process")
    ...
    def _wait_for_response(self, timeout: Optional[float] = None) -> Any:
        if not self._save_process.is_alive():
            logger.info(f?"Checkpoint background process is dead calling join()")
            self._save_process.join()
            raise RuntimeError(
    ...
    def _checkpointing_subprocess(
        ...
        try:
            ...
            logger.info(f?"Checkpoint background process is running")
            parent_conn.send(_CheckpointSaveProcessControlOpts.INIT_COMPLETE)

            # Serving loop.
            while True:
                logger.info(f?"Waiting for checkpoint save request")
            ...
            logger.info(f?"Checkpoint background process is shutting down")
