...
export default function NoteNinjas() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      // Generate recommendations from backend
      ...
      try {
        ...
        recommendations = await noteNinjasAPI.generateRecommendations(
          userInputStr,
          {}, // RAG manifest (optional)
          `session_${Date.now()}` // session ID
        );
      } catch (error) {
        console.error("Error generating recommendations:", error);
        // Continue anyway - will use static suggestions
      }
